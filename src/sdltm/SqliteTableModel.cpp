...
void SqliteTableModel::SetUpdateCache(const SdltmUpdateCache& updateCache) {
...
QVariant SqliteTableModel::data(const QModelIndex &index, int role) const
{
	...
	auto isSegmentSource = column == 1;
	auto isSegmentTarget = column == 2;
	if (role == Qt::DisplayRole && _updateCache != nullptr && row_available && (isSegmentSource || isSegmentTarget)) {
		auto id = m_cache.at(row).at(0).toInt();
		if (id > 0 && _updateCache->Has(id, isSegmentSource ? SdltmUpdateCache::ValueType::Source : SdltmUpdateCache::ValueType::Target)) {
			auto text = _updateCache->GetFriendlyText(id, isSegmentSource ? SdltmUpdateCache::ValueType::Source : SdltmUpdateCache::ValueType::Target);
